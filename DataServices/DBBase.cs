...
namespace DataServices
{
    ...
    {
        public abstract string RemoteCs { get; set; }
        public abstract string LocalCs { get; set; }
        protected abstract MySqlConnection ConRemote { get; set; }
        //protected abstract MySqlConnection conLocal { get; set; }
        public abstract void SetConRemote();
        public abstract void SetConLocal(string con);
        public abstract bool TestConnection(string cs);
        public abstract bool TruncateTables();
    }
    public static class Encrypt
    {
        ...
        public static string DecryptString(string cipherText, string passPhrase)
        {
            try
            {
                ...
                memoryStream.Close();
                cryptoStream.Close();
                string aaaaaaa = Encoding.UTF8.GetString(plainTextBytes, 0, decryptedByteCount);
                return aaaaaaa;
            }
            catch (Exception)
            {
                //MessageBox.Show($"{cipherText} {EX.Message}", "HATA");
                return "";
            }
