    ...
    public class PersonelimDegil : TrmBase
    {
        ...
        public bool DoWork(List<Visit> kontrolEdilecekler, out string msg)
        {
            ...
            try
            {
                ...
                {
                    if(GlobalVars.CancelProcess == true) { return false; }
                    IseGirisCikis.Clear(); GirisCikisIslemleri.Clear();
                    CikisGiris.Clear();
        ...
        private bool IstenCikisTara(Visit personel, out string msg) // true : işten çıkmış ve geri dönmemiş
        {
            ...
            try
            {
                ...
                {
                    if (GlobalVars.CancelProcess == true) { msg = "iptal"; return false; }
                    IWebElement linkAnaMenu = waitForLink.Until(SeleniumExtras.WaitHelpers.ExpectedConditions.ElementExists(By.LinkText("Ana Menü")));
                    linkAnaMenu.Click();
                }
                ...
                {
                    if (GlobalVars.CancelProcess == true) { msg = "iptal"; return false; }
                    linkAyrilis = waitForLink.Until(SeleniumExtras.WaitHelpers.ExpectedConditions.ElementExists(By.LinkText("İŞTEN AYRILIŞ GÖRÜNTÜLEME")));
                    linkAyrilis.Click();
                }
                ...
                if (sonCikisTarihi != DateTime.MinValue) // Eğer en az bir kez çıkış yapılmış ise
                {
                    ...
                    {
                        //if (i != 1)// ilk tarih işe ilk giriş tarihi olduğu için döngünün ilk adımında işlem yapmadan pas geç
                        //{
        ...
        private DateTime ProcessCheckLeaveForTcno(Visit personel, out string msg)
        {
            ...
            try
            {
                ...
                {
                    if (GlobalVars.CancelProcess == true) { msg = "iptal"; return new DateTime(2222,2,2); }
                    IWebElement linkAnaMenu = wait.Until(SeleniumExtras.WaitHelpers.ExpectedConditions.ElementExists(By.LinkText("Ana Menü")));
                    linkAnaMenu.Click();
                }
                pageHtml = Surucu.Driver.PageSource;
                ...
                {
                    if (GlobalVars.CancelProcess == true) { msg = "iptal"; return new DateTime(2222, 2, 2); }
                    IWebElement linkAyrilis = wait.Until(SeleniumExtras.WaitHelpers.ExpectedConditions.ElementExists(By.LinkText("İŞTEN AYRILIŞ GÖRÜNTÜLEME")));
                    linkAyrilis.Click();
                }
                if (GlobalVars.CancelProcess == true) { msg = "iptal"; return new DateTime(2222, 2, 2); }
                var elementTcnoField = wait.Until(SeleniumExtras.WaitHelpers.ExpectedConditions.ElementToBeClickable(Surucu.Driver.FindElement(By.Name("kimlikno"))));
                ...
                var element = wait.Until(SeleniumExtras.WaitHelpers.ExpectedConditions.ElementToBeClickable(sorgulaButton));
                if (GlobalVars.CancelProcess == true) { msg = "iptal"; return new DateTime(2222, 2, 2); }
                sorgulaButton.Click();
        ...
        private bool ProcessCheckHireForTcno(Visit personel, out string msg) // false: ise işten çıkmış ve geri dönmemiş
        {
            ...
            try
            {
                ...
                if (GlobalVars.CancelProcess == true) { return false; }
                IWebElement linkIseGiris = wait.Until(SeleniumExtras.WaitHelpers.ExpectedConditions.ElementExists(By.LinkText("İŞE GİRİŞ GÖRÜNTÜLEME")));
                linkIseGiris.Click();
                ...
                var rows = table.FindElements(By.TagName("tr"));
                int rowCount = rows.Count;
                wait.Until(SeleniumExtras.WaitHelpers.ExpectedConditions.ElementIsVisible(By.XPath("/html/body/table[3]/tbody/tr/td/center/table/tbody/tr[2]/td[2]/form/span/table/tbody/tr[6]/td[5]")));
